...
- **安全防御**：管理IP拦截和白名单
- **告警管理**：查看和处理系统告警
- **AI模型管理**：查看AI模型状态和性能，支持数据集上传和模型训练
- **系统设置**：管理系统配置和用户信息
...
- `POST /api/ai/upload-dataset` - 上传数据集并启动训练（支持CSV和JSON格式）
...
### 支持的数据集格式
- **CSV格式**：支持CICIDS2017等标准网络流量数据集
- **JSON格式**：支持自定义JSON格式的流量数据

### 前端上传流程
1. 登录系统，进入「AI智能检测引擎」页面
2. 在「数据集训练与热更新」区域，拖拽CSV/JSON文件到上传区域或点击选择文件
3. 可选：指定模型版本名称
4. 系统自动上传文件并启动训练任务
5. 训练完成后，系统会自动切换到新版本模型

### 后端训练流程
1. 接收上传的数据集文件
2. 保存到 `backend/data/datasets/` 目录
3. 启动异步训练进程
4. 训练完成后更新模型状态
...
11. **数据集训练**：支持从CSV和JSON数据集训练模型，提高检测准确率
12. **热更新**：模型训练完成后自动热更新，无需重启服务